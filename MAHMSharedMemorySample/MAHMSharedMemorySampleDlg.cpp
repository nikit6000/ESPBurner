...
BOOL CMAHMSharedMemorySampleDlg::OnInitDialog()
{
	...
	/* starting web server */
	//std::thread webServerThread([=] { NKStartWebServer(); });
	NKStartWebServer();
...
void CMAHMSharedMemorySampleDlg::OnTimer(UINT nIDEvent) 
{
	...
	g_httpHandler->setUpdatingStatus(true);
			...
			//check if we're connected to valid memory
		{
				...
				//GPU entries are available only in v2.0 and newer shared memory format
			{
					...
					//display info for each GPU
				{
					...
					NodeGPU * nodeGPU = new NodeGPU(dwGpu, lpGpuEntry);
					...
					if (g_httpHandler->nodes.find(nodeGPU->getName()) != g_httpHandler->nodes.end()) {
						if (g_httpHandler->nodes[nodeGPU->getName()]->getBusy()) {
							g_httpHandler->nodes[nodeGPU->getName()]->selfDistruct = true;
						}
						else {
							delete g_httpHandler->nodes[nodeGPU->getName()];
						}
					}
					g_httpHandler->nodes[nodeGPU->getName()] = nodeGPU;
				...
				NodeObject * nodeGlobal = new NodeObject(L"global");
				for (DWORD dwSource=0; dwSource<dwSources; dwSource++)
					//display info for each data source
				{
					LPMAHM_SHARED_MEMORY_ENTRY	lpEntry		= (LPMAHM_SHARED_MEMORY_ENTRY)((LPBYTE)lpHeader + lpHeader->dwHeaderSize + dwSource * lpHeader->dwEntrySize);
					...
					nodeGlobal->processEntry(lpEntry);
					strStatus += DumpEntry(lpEntry);
						//dump data source entry
				}
				...
				if (!strStatus.IsEmpty())
				{
					...
					m_strStatus += strStatus;
					m_strStatus += "\n";
					if (g_httpHandler->nodes.find(nodeGlobal->getName()) != g_httpHandler->nodes.end()) {
						if (g_httpHandler->nodes[nodeGlobal->getName()]->getBusy()) {
							g_httpHandler->nodes[nodeGlobal->getName()]->selfDistruct = true;
						} else {
							delete g_httpHandler->nodes[nodeGlobal->getName()];
						}
					}
					g_httpHandler->nodes[nodeGlobal->getName()] = nodeGlobal;
				}
			}
			else
			{
	...
	g_httpHandler->setUpdatingStatus(false);
...
void CMAHMSharedMemorySampleDlg::NKStartWebServer() {
	utility::string_t port = U("8085");

	utility::string_t address = U("http://192.168.0.107:");
	address.append(port);

	OnWebInitialize(address);
}

void CMAHMSharedMemorySampleDlg::OnWebInitialize(const string_t& address)
{
	uri_builder uri(address);


	auto addr = uri.to_uri().to_string();
	g_httpHandler = std::unique_ptr<WebHandler>(new WebHandler(addr));
	g_httpHandler->open().wait();

}

void CMAHMSharedMemorySampleDlg::OnWebShutdown()
{
}
